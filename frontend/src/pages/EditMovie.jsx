...
import { ArrowLeft, Save, Eye, AlertCircle, CheckCircle, Loader2, X } from 'lucide-react'
...
const EditMovie = () => {
    ...
    const fetchMovie = async () => {
      try {
        ...
        setFormData({
          ...
          imdbRating: movie.imdbRating === 'N/A' ? '' : movie.imdbRating || '',
          genre: movie.genre || [],
          ...
          story: movie.story || '',
          downloadLinks: {
            '1080p': movie.downloadLinks?.['1080p'] || '',
            '720p': movie.downloadLinks?.['720p'] || '',
            '480p': movie.downloadLinks?.['480p'] || ''
          }
        })
      } catch (err) {
        showError(`Failed to fetch movie: ${err.message}`)
        navigate('/')
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showError(`Failed to update movie: ${error.message}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  if (initialLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900">
        <LoadingSpinner size="lg" text="Loading movie data..." />
      </div>
    )
  }
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-white">Edit Movie</h1>
          </div>
            ...
            <div className="lg:col-span-1 space-y-6">
              ...
              <div className="bg-gray-800 rounded-xl p-6">
                <div className="space-y-3">
                  ...
                  <Link
                    to="/"
                    className="w-full block text-center px-4 py-3 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors font-medium"
                  >
                    Cancel
                  </Link>
                </div>
